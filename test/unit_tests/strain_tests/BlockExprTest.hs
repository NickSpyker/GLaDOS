...
import BlockExpr (BExpr(..), tokensToBlock, handleSection)
...
testToBlock = 
  ...
  [
    ...
    "Empty section parsing"
    ~: case tokensToBlock "testmodule" [TokVar, TokSmCol] of
         Right (BModule "testmodule" [BSection [T TokVar]]) -> True
         _ -> False
    ~?= True,

    "Section parsing with following valid expressions"
    ~: case tokensToBlock "testmodule" [TokVar, TokSmCol, TokVar] of
         Right (BModule "testmodule" [BSection [T TokVar], T TokVar]) -> True
         _ -> False
    ~?= True,

    "Section parsing with semicolon at the end"
    ~: case tokensToBlock "testmodule" [TokVar, TokSmCol, TokVar, TokSmCol] of
         Right (BModule "testmodule" [BSection [T TokVar], BSection [T TokVar]]) -> True
         _ -> False
    ~?= True
  ]